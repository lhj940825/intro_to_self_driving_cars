...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	vector<char>::size_type num_rows, num_cols;
	num_rows = grid.size();
	num_cols = grid[0].size();
	vector< vector <float> > newGrid = zeros(num_rows, num_cols);
	
	float init_prob = 1.0 / (num_rows * num_cols); 

	for(int i = 0; i < num_rows; i++){
		for(int j = 0; j < num_cols; j++){
			newGrid[i][j] = init_prob;
		}
	}
...
vector< vector <float> > move(int dy, int dx, 
  ...
  float blurring) {
	vector<float>::size_type num_rows, num_cols;
	num_rows = beliefs.size();
	num_cols = beliefs[0].size();
  	vector < vector <float> > newGrid = zeros(num_rows, num_cols);

	for(int i = 0; i < num_rows; i++){
		for(int j = 0; j < num_cols; j++){
			int new_i, new_j;
			new_i = (i + dx + num_rows)% num_rows;
			new_j = (j + dy + num_cols)% num_cols;
...
vector< vector <float> > sense(char color, 
	...
	float p_miss) 
{
	vector<int>::size_type num_rows, num_cols;
	num_rows = grid.size();
	num_cols = grid[0].size();
	vector< vector <float> > newGrid = zeros(num_rows, num_cols);
