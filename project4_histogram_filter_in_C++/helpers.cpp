...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// todo - your code here
	vector<float>::size_type num_rows = grid.size();
	vector<float>::size_type num_cols = grid[0].size();
	float normalizer = 0.0;

	vector< vector<float> > newGrid = zeros(num_rows, num_cols);

	// compute the normalization factor and paste the values of grid into newGrid
	for(vector<float>::size_type i = 0; i < num_rows; i++){
		for(vector<float>::size_type j = 0; j < num_cols; j++){
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	vector < vector <float> > blurWindow = {  {corner_prob,   adjacent_prob,  corner_prob},
											  {adjacent_prob, center_prob,    adjacent_prob},
											  {corner_prob,   adjacent_prob,  corner_prob}};

	vector<int> list_dx = {-1, 0, 1};
	vector<int> list_dy = {-1, 0, 1};
	...
	for(int i = 0; i < num_rows; i++) {
		for(int j = 0; j < num_cols; j++) {
			...
			for(int ii=0; ii < list_dx.size(); ii++) {
				for(int jj = 0; jj < list_dy.size(); jj++){
					int dx = list_dx[ii];
					int dy = list_dy[jj];

					int dx_i = (i + dx + num_rows) % num_rows;
					int dy_j = (j + dy + num_cols) % num_cols;
					newGrid[i][j] += grid[dx_i][dy_j] * blurWindow[ii][jj];
				}
			}
